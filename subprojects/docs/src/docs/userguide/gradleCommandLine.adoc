...
`--warnings`::
Specifies which type of Gradle deprecation warnings displayed in console.
+
Set to `all` to render all deprecation warnings. This may be useful for users who're concern about these warnings.
+
Set to `summary` (the default) to suppress all deprecation warnings in build but the "deprecation warning summary": `There're X deprecation warnings in your build.`
+
Set to `none` to suppress all deprecation warnings as if they don't exist at all.
`--continue`::
